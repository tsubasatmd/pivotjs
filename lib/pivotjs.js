...
(function() {
  ...
  Pivot = (function() {
    ...
    function Pivot(param) {
      ...
      this.defaultFormat = this.formats.int;
      ...
      _.each(this.measureAttrs, (function(_this) {
        return function(measure) {
          if (_.has(measure, 'format')) {
            return measure.formatExpression = _.has(_this.formats, measure.format) ? _this.formats[measure.format] : measure.format;
          } else {
            return measure.format = _this.formats[_this.defaultFormat];
          }
        };
      })(this));
      this.showTotal = (ref = this.param.showTotal) === null || ref === (void 0) ? true : this.param.showTotal;
      this.showSubTotal = (ref1 = this.param.showSubTotal) === null || ref1 === (void 0) ? true : this.param.showSubTotal;
      this.rowKeys = [];
      this.colKeys = [];
      this.measureKeys = [];
    ...
    Pivot.prototype.getSortedRowKeys = function() {
      return this.sortedRowKeys || (this.sortedRowKeys = this.getSortedKeys(this.rowKeys, this.rowAttrs));
    };
    ...
    Pivot.prototype.getSortedColKeys = function() {
      return this.sortedColKeys || (this.sortedColKeys = this.getSortedKeys(this.colKeys, this.colAttrs));
    };
    ...
    Pivot.prototype.getShowSubTotal = function() {
      return this.showSubTotal || false;
    };

    Pivot.prototype.getShowTotal = function() {
      return this.showTotal || false;
    };

    Pivot.prototype.getSortedKeys = function(keys, attrs) {
      var sort;
      if (keys == null) {
        keys = [];
      }
            ...
            diff = (function() {
              ...
              switch (type) {
                ...
                case 'measure':
                  ...
                  _key = sortObject.key;
                  pos = sortObject.position || 0;
                  ref1 = sortObject.kind === 'row' ? [[key_a, _key], [key_b, _key]] : [[_key, key_a], [_key, key_b]], args_a = ref1[0], args_b = ref1[1];
    ...
    Pivot.prototype.processRecord = function(record) {
      var c, colKeys, i, index, j, k, key, len, len1, len2, measure, r, ref, ref1, ref2, ref3, ref4, rowKeys, serializedColKey, serializedRowKey, serializedRowKeyList, serializedSlicedRowKeys, slicedRowKeys;
      colKeys = (function() {
      ...
      if (colKeys.length !== 0) {
        ...
        if (!this.colTotals[serializedColKey]) {
          this.colTotals[serializedColKey] = new Composer(this, [], colKeys);
        }
      }
      serializedRowKeyList = [];
      for (index = i = 0, len = rowKeys.length; i < len; index = ++i) {
        key = rowKeys[index];
        ...
        if (rowKeys.length !== 0) {
          ...
          if (colKeys.length !== 0) {
            ...
            if (!(serializedColKey in this.map[serializedSlicedRowKeys])) {
              this.map[serializedSlicedRowKeys][serializedColKey] = new Composer(this, slicedRowKeys, colKeys);
            }
          }
        }
      }
      ...
      for (j = 0, len1 = ref.length; j < len1; j++) {
        ...
        if ((ref1 = this.colTotals[serializedColKey]) != null) {
          ref1.add(measure, record);
        }
        for (k = 0, len2 = serializedRowKeyList.length; k < len2; k++) {
          key = serializedRowKeyList[k];
          if ((ref2 = this.rowTotals[key]) != null) {
            ref2.add(measure, record);
          }
          if ((ref3 = this.map[key]) != null) {
            if ((ref4 = ref3[serializedColKey]) != null) {
  ...
  Aggregator = (function() {
    ...
    Aggregator.prototype.value = function() {
      ...
      if (this.measure.aggregation) {
        ...
        try {
          return this.val = this.evaluate(jsep(this.measure.expression));
        } catch (error) {
          e = error;
          return this.val = null;
        }
    ...
    Aggregator.prototype.formattedValue = function() {
      ...
      return this.composer.pivot.formatFunction(_val, this.measure.formatExpression || this.composer.pivot.defaultFormat);
    };

    Aggregator.prototype.evaluate = function(node) {
      var agg, key, left, pos, right;
      switch (node.type) {
        ...
        case 'MemberExpression':
          if (node.type) {
            return this.composer.value(node.object.name, node.property.name);
          } else {
            throw new Error('Unsupported Cascading MemberExpression');
          }
          break;
        case 'UnaryExpression':
          if (node.argument.type !== 'Literal' && node.argument.type !== 'MemberExpression') {
            return new Error('Unsupported argument');
          }
          switch (node.operator) {
            case '@':
              if (node.argument.type === 'MemberExpression') {
                if (node.argument.object.type === 'Identifier') {
                  key = node.argument.object.name;
                  agg = node.argument.property.name;
                  pos = 0;
                } else if (node.argument.object.type === 'MemberExpression') {
                  key = node.argument.object.object.name;
                  agg = node.argument.object.property.name;
                  pos = this.evaluate(node.argument.property);
                } else {
                  throw new Error('');
                }
              }
              return this.composer.value(key, agg, pos);
            case '-':
              return -1 * node.argument.value;
            case '+':
