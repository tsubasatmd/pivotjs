...
(function() {
  ...
  Pivot = (function() {
    ...
    Pivot.prototype.processRecord = function(record) {
      var c, cKey, cKeys, ckey, colKeyList, colKeys, i, index, j, k, key, l, len, len1, len2, len3, len4, len5, len6, m, measure, o, p, r, ref, ref1, ref2, ref3, ref4, rkey, rowKeys, serializedColKey, serializedColKeysList, serializedRowKey, serializedRowKeyList, serializedSlicedColKeys, serializedSlicedRowKeys, slicedColKeys, slicedRowKeys;
      rowKeys = (function() {
      ...
      for (l = 0, len3 = ref.length; l < len3; l++) {
        ...
        for (m = 0, len4 = colKeyList.length; m < len4; m++) {
          ...
          for (o = 0, len5 = serializedRowKeyList.length; o < len5; o++) {
            ...
            if ((ref2 = this.map[rkey]) != null) {
              if ((ref3 = ref2[ckey]) != null) {
                ref3.add(measure, record);
              }
            }
          }
        }
        for (p = 0, len6 = serializedRowKeyList.length; p < len6; p++) {
          rkey = serializedRowKeyList[p];
          if ((ref4 = this.rowTotals[rkey]) != null) {
    ...
    sum = function(measureKey, aggregator) {
      var notEmptyRecords;
      notEmptyRecords = _.filter(aggregator.records, function(record) {
        return !!record[measureKey] || record[measureKey] === 0;
      });
      if (notEmptyRecords.length === 0) {
        return null;
      }
      return _.reduce(notEmptyRecords, function(summed, record) {
    ...
    average = function(measureKey, aggregator) {
      return (sum(measureKey, aggregator) / counta(measureKey, aggregator)) || null;
    };
    ...
    min = function(measureKey, aggregator) {
      ...
      record = _.minBy(notEmptyRecords, function(record) {
        return parseFloat(record[measureKey]) || 0;
      });
      if (record[measureKey] || record[measureKey] === 0) {
        return record[measureKey];
      } else {
        return null;
      }
      ...
      return _.map(this.aggregators, (function(_this) {
        return function(agg) {
          ...
          return {
            ...
            value: ((ref1 = agg.aggregator) != null ? ref1.value() : void 0) || ((ref2 = agg.aggregator) != null ? ref2.value() : void 0) === 0 ? agg.aggregator.value() : null
          };
        };
      })(this));
    };
